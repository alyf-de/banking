...
banking.bank_reconciliation.prompt_manual_reconcile_amounts = function (
	context,
	transaction,
	selected_voucher,
) {
	return new Promise((resolve, reject) => {
		...
		const max_source_amount = Math.max(0, flt(transaction.unallocated_amount));
		const voucher_outstanding_amount = Math.abs(flt(selected_voucher?.amount));
		...
		const sync_target_with_source_and_rate = async (source_input = null) => {
			...
			if (source_input !== null && !same_number(source_input, source_amount)) {
